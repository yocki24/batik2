...
{{ Form::label('month', __('laporan_bulanan.view_monthly_label'), ['class' => 'control-label']) }}
...
{{ link_to_route('laporan_bulanan.sales.monthly', __('laporan_bulanan.this_month'), [], ['class' => 'btn btn-default btn-sm']) }}
{{ link_to_route('laporan_bulanan.sales.yearly', __('laporan_bulanan.view_yearly'), ['year' => $year], ['class' => 'btn btn-default btn-sm']) }}
{{ Form::close() }}

<div class="panel panel-primary">
    <div class="panel-heading"><h3 class="panel-title">{{ __('laporan_bulanan.sales_graph') }} {{ $months[$month] }} {{ $year }}</h3></div>
        ...
        <table class="table table-condensed">
                ...
                @if ($any)
                    <tr>
                        ...
                        <td class="text-center">
                            {{ link_to_route(
                                'laporan_bulanan.daily',
                                __('laporan_bulanan.view_daily'),
                                ['date' => $date],
                                [
                                    'class' => 'btn btn-info btn-xs',
                                    'title' => __('laporan_bulanan.daily', ['date' => date_id($date)]),
                                ]
                            ) }}
                        </td>
                    </tr>
            ...
            <tfoot>
                <tr>
                    ...
                    <th class="text-center">{{ $laporan_bulanan->sum('count') }}</th>
                    <th class="text-right">{{ format_rp($laporan_bulanan->sum('amount')) }}</th>
                    <td>&nbsp;</td>
                </tr>
            </tfoot>
        </table>

...
@section('content')
	<div class="row">
		<div class="col-md-5">
            ...
            @else
                <img class="img-thumbnail img-fluid" src="{{ asset('/storage/.$product->image') }}/{{ $item->gambar }}" alt="Card image cap">
            @endif
		</div>
		<div class="col-md-7">
          <div class="card card-stats mb-4 mb-xl-0">
            <div class="card-body">
    			...
    			<div class="mt-4" style="width: 60%">
    				<table class="table">
        				<tr>
        					<th><h3>Harga</h3></th>
        					<th><h3>:</h3></th>
        					<th><h3>Rp. {{ number_format($item->price, 0, '.', '.') }}</h3></th>
        				</tr>
        				<tr>
        					<th><h3>Terjual</h3></th>
        					<th><h3>:</h3></th>
        					<th><h3>{{ number_format($terjual, 0, '.', '.') }} {{ $item->stok }}</h3></th>
        				</tr>
        			</table>
    			</div>
    			<div class="mt-4">
    			...
    			<div>
    				<a href="{{ url('/pengrajin/produk') }}" class="mt-2 btn btn-dark float-right">Kembali</a>
    			</div>
            </div>
          </div>
        </div>
